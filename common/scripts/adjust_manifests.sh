...
if [[ -f "${CSV_PATH}" ]]; then
    ...
    yq w ${CSV_PATH} "spec.relatedImages[0] image" "icr.io/cpopen/cpfs/ibm-crossplane-bedrock-shim-config:${OPERATOR_VERSION}" 1<>${CSV_PATH}
    yq w ${CSV_PATH} "spec.relatedImages[1] name" "IBM_CROSSPLANE_OPERATOR_IMAGE" 1<>${CSV_PATH}
    yq w ${CSV_PATH} "spec.relatedImages[1] image" "icr.io/cpopen/ibm-crossplane-operator:${OPERATOR_VERSION}" 1<>${CSV_PATH}
